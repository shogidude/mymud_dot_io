...
The `home-routes` handler in the `mymud-dot-io.routes.home` namespace
...
The `home-page` function will in turn call the `mymud-dot-io.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `mymud-dot-io.handler` namespace:
...
Request middleware functions are located under the `mymud_dot_io.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `mymud-dot-io.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `mymud-dot-io.db.core` in some other namespace.
